...
namespace Evie.Titanium
{
    public class SpellFileRecord
    {
        ...
        {
            public SpellFileRecord s { get; set; }
            public string name { get; set; }
            public FieldIndexer(SpellFileRecord spellFileRecord, string name)
            {
                this.s = spellFileRecord;
                this.name = name;
            }
            ...
            public int this[int index]
            {
                get
                {
                    switch (name)
                    {
                        ...
                        default:
                            throw new ArgumentException(String.Format(CultureInfo.InvariantCulture, "Unknown indexer name '{0}'", name));
                    }
                    return 0;
                }
            }
